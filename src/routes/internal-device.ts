...
</head><body><div class="card"><h1>Codex Device Login</h1><p class="hint">Dev POC only: submitting this form requires the dashboard session today; Feishu approval should replace that gate.</p><form method="post" action="/codex/device"><label>One-time code</label><input name="user_code" value="${escapedCode}" autofocus><button type="submit">Approve dev login</button></form>${escapedMessage ? `<p class="msg">${escapedMessage}</p>` : ""}</div></body></html>`;
}
