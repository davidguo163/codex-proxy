...
- `/v1/models` 现在返回 `context_window` 和 `max_output_tokens` 字段：读取 `models.yaml` / 动态后端的 `maxContextWindow ?? contextWindow`，帮助 OpenCode 等依赖该字段判断上下文窗口的客户端正确识别上限（`src/routes/models.ts`、`src/types/openai.ts`）（closes #578）
...
- CI Quality gate 新增 `test-build-typecheck` job：每次 PR 现在自动跑全量单测 / integration / e2e（239 个测试文件）、`tsc --noEmit` 全源码类型检查、`npm run build:web` 前端构建，补齐 CI 覆盖缺口（`.github/workflows/ci-quality.yml`）（closes #378）
